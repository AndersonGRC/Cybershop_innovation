...
class EmergencyIntegrationTests(unittest.TestCase):
    ...
    def test_model_is_a_dropdown_that_keeps_the_current_value(self):
        ...
        with patch.object(integrations, 'models_cached', return_value=['gpt-oss-cyber', 'qwen2.5:7b']):
            fields = {field['key']: field for group in integrations.get_integrations('cliente-a')
                      for field in group['fields']}
        self.assertEqual(fields['AI_MODEL']['type'], 'modelos')
        self.assertEqual(fields['AI_MODEL']['options'], ['modelo-viejo', 'gpt-oss-cyber', 'qwen2.5:7b'])
        self.assertEqual(fields['AI_MODEL']['value'], 'modelo-viejo')
